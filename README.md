...
[![Game Screenshot](screenshots/final_screenshot.png)](https://github.com/Mimic52006masaki/DeployYourLife)
...
- プロダクト別アサインで役割発揮（Developer/Designer/Marketer）
- チーム編成で高額案件対応
...
- プロダクト別アサイン: Developer/Designer/Marketer 役割発揮
...
**目標: 12ヶ月でエンジニアとして成功を収める！**

### 📚 フェーズごとの成長
1. **学生フェーズ**: 学習とSNSで基礎固め
2. **バイトフェーズ**: 副業で収入増 + 個人開発開始
3. **会社員フェーズ**: 安定収入だが時間制限（行動-1）
4. **フリーランスフェーズ**: 社員雇用・プロダクト事業化
5. **法人フェーズ**: 組織経営・自動収益化

### 🎯 戦略のポイント
- **スキル習得**: 案件解禁・開発可能範囲拡大
- **SNS運用**: フォロワー増加で収益倍率UP
- **個人開発**: プロダクト作成・改善・収益化
- **社員管理**: アサインで役割発揮・失敗回避
- **リスク管理**: 品質維持・炎上防止・精神管理
...
| v0.6     | 社員アサインシステム / role別効果 / プロダクト別配置                     |
| v0.7     | プロダクト失敗ルート / 品質管理 / 炎上リスク / 自然離脱                  |
| v0.7.1   | 設定画面 + GameStateContext 連動版 / 独学モード vs 投資モード / UI/UX改善     |
...
- **収益ロジック**: hasPayment + quality + followers + users + buzz + role効果
- **ペナルティ**: deploy で quality -2（現実的なリリースリスク）

### 👥 社員アサインシステム
- **データ構造**: employee.assignedProductId
- **役割効果**: Developer (quality/バグ修正), Designer (UI/安定性), Marketer (成長/バズ)
- **配置戦略**: 各プロダクトに個別アサイン（1社員=1プロダクト）
- **UI**: 社員カードからプロダクト選択モーダル

### 🔥 プロダクト失敗ルート
- **品質低下**: quality < 10 で5%解約（毎月）
- **炎上リスク**: users > 1000 & quality < 20 で10%確率炎上（-20% users）
- **自然離脱**: age > 6 で15%確率ユーザ減少（古いアプリ）
- **ペナルティ回避**: 社員配置でリスク低減

### 🧠 改善アクション
- **FIX BUG**: quality +5〜10 (+ Developerボーナス), mental +10
- **UI IMPROVE**: quality +3 (+ Designerボーナス), users +10, money -30,000
- **MARKETING**: 成功率 + Marketerボーナス, money -30,000
- **プロダクト別適用**: released+ の各アプリに個別実行

### 📢 マーケティングシステム
- **プロダクト別**: 各アプリにマーケティング施策実行
- **効果**: 成功/通常/失敗 のランダム結果 + Marketerボーナス
...
- **スキルボーナス**: quality / 20
- **バズ倍率**: ランダムイベント
- **role効果**: Developer +10%, Marketer +20%, Designer +5%

### 🎨 UI/UX改善
- **プロダクトリスト**: stage 別表示 + 条件付きボタン + アサイン社員表示
- **社員カード**: アサイン状態表示 + アサインボタン
- **ログ**: 詳細なフィードバック（アプリ名・効果・失敗警告）
- **フェーズ表示**: 現在の制限状態を明確化

### ⚙️ 設定システム
- **設定画面**: 初期状態・学習コスト・アクション数設定可能
- **独学 vs 投資モード**: 無料低速 vs 有料高速 戦略選択
- **コストベーススキル上昇**: 投資額に応じた動的成長

### 🎯 UI/UX 最適化
- **ログ上部移動**: HUD 下に常時表示でアクション即確認
- **HUD 強調**: 背景・境界・アイコンで重要情報視認性向上
- **収支グラフ常時表示**: データ有時右カラムに収入・支出可視化
- **非操作画面簡略化**: Game Over/End Game で余計な情報非表示
- **サマリー最小化**: 月報を簡潔表示（収入・支出・収支・現在金・メンタル・フォロワー）
